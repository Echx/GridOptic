...
class GameTestViewController: UIViewController {
    ...
    private func setUpGrid() {
        let mirror = GOFlatMirrorRep(center: GOCoordinate(x: 60, y: 24), thickness: 2, length: 6, direction: CGVectorMake(0.1, 1), id: "MIRROR_1")
        self.grid?.addInstrument(mirror)
        let flatLens = GOFlatLensRep(center: GOCoordinate(x: 28, y: 32), thickness: 8, length: 8, direction: CGVectorMake(1, 0), refractionIndex: 1.33, id: "FLAT_LENS_1")
        self.grid?.addInstrument(flatLens)
        let concaveLens = GOConcaveLensRep(center: GOCoordinate(x: 44, y: 25), direction: CGVectorMake(0, 1), thicknessCenter: 1, thicknessEdge: 4, curvatureRadius: 20, id: "CONCAVE_LENS_1", refractionIndex: 1.50)
